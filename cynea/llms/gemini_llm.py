...
from cynea.helpers.utils import now_ms, compute_function_pre_call_message, convert_to_request_log
...
class GeminiLLM(BaseLLM):
    ...
    def __init__(self, max_tokens=100, buffer_size=40, model="gemini-2.5-flash", temperature=0.1, **kwargs):
        ...
        cynea_tools = kwargs.get("api_tools", {}).get("tools", [])

        gemini_declarations = []
        if cynea_tools:
            if isinstance(cynea_tools, str):
                try:
                    cynea_tools = json.loads(cynea_tools)
                except json.JSONDecodeError:
                    logger.error("Failed to parse tool definitions as JSON")
                    cynea_tools = []

            for tool in cynea_tools:
        ...
        # Keep raw cynea tools list for required-param validation at call time
        self.cynea_tools_raw = cynea_tools if isinstance(cynea_tools, list) else []
    ...
    def _prepare_history(self, messages):
        """Translate cynea roles (OpenAI-style) to Gemini-style roles and parts."""
        system_instruction = ""
        history = []
