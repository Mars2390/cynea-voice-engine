...
from cynea.helpers.function_calling_helpers import trigger_api, computed_api_response, prepare_api_request
...
from cynea.agent_types import *
from cynea.providers import *
from cynea.enums import TelephonyProvider, LogComponent, LogDirection, HangupReason, NodeType
from cynea.exceptions import cyneaComponentError, LLMError, SynthesizerError, TranscriberError
from cynea.prompts import *
from cynea.helpers.language_detector import LanguageDetector
from cynea.transcriber.transcriber_pool import TranscriberPool
from cynea.synthesizer.synthesizer_pool import SynthesizerPool
from cynea.helpers.utils import (
...
class TaskManager(BaseManager):
    ...
    async def _run_llm_task(self, message):
        ...
        try:
            ...
            self.llm_task = None
        except cyneaComponentError as e:
            self.response_in_pipeline = False
    ...
    async def _end_call_on_component_error(self, error, hangup_detail):
        ...
        if self.run_id and not self._error_logged:
            if isinstance(error, cyneaComponentError):
        ...
        try:
            if self._is_conversation_task():
                ...
                try:
                    ...
                    if not isinstance(e, cyneaComponentError):
                        logger.error(f"Error: {e}")
                    if self.run_id and not self._error_logged:
                        if isinstance(e, cyneaComponentError):
                ...
                if _stored_err is not None:
                    ...
                    if issubclass(err_cls, cyneaComponentError):
                        raise err_cls(
